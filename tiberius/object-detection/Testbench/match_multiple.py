...
if ((RED_PIXELS > PIXEL_THRESHOLD) or (BLUE_PIXELS > PIXEL_THRESHOLD) or (GREEN_PIXELS > PIXEL_THRESHOLD)):
    ...
    # If the image wasn't cleared (no descriptors) - when the picture doesn't
    # match the desired object.
    if (test_dpts is not None):
        ...
        for image, descriptor in libr_imgs.iteritems():
            ...
            if (image == 'cube'):
                cube_rate = match_rate
            elif (image == 'hexagon'):
                hexagon_rate = match_rate
            elif (image == 'star'):
                star_rate = match_rate
