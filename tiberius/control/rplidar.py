...
class RoboPeakLidar:
    ...
    def getlines(self):
        ...
        # converts lidar data from polar to rectangular coordinates and stores
        # in 'dots' list
        ...
        # loops through a list of dots and groups them into lines based on
        # 'gap' and 'curvfront' parameters
        for x in xrange(dotsnumb - 1):
            # if a current row in 'lines' is empty
            if (lines[lncnt][0] == 0):
                if (dist(dots[dtcnt][0], dots[dtcnt][1], dots[
                        dtcnt + 1][0], dots[dtcnt + 1][1]) < gap):
                    # starts a new line with a fixed start which is the current dot and end is the next dot,
                    # which may or may not be changed later based on the next
                    # dots (not)being in the same line
                ...
                # creates two vectors of the same length as gapvctpol, which
                # are spread from it by +-'curvfront' degrees
                ...
                # and terminating at boundhigh and boundlow, creating an angle
                # of (2*curvfront)
                ...
                # here the code can be added which introduces the use of
                # 'curvrear' parameter

                # if the next dot is in the above described area then the current line end's coordinates are replaced by
                # the next dote coordinates, so the next dot becomes the end of
                # the current line
                if ((((linehigh < 0 and linelow > 0) or (linehigh > 0 and linelow < 0)) and
                    ...
                    lines[lncnt][3] = dots[dtcnt + 1][1]
                    dtcnt = dtcnt + 1
                # otherwise the current line stays as it is, and a new empty
                # line is created
                else:
                    lines.append([0, 0, 0, 0])
                    lncnt = lncnt + 1
        ...
        if (lines[len(lines) - 1][0] != 0):
            # check if the last line is not a dot, since vector of a dot is 0,
            # then apply the same process as above
            if (lines[len(lines) - 1][2] != lines[len(lines) - 1][0]
                    and lines[len(lines) - 1][3] != lines[len(lines) - 1][1]):
                ...
                boundlow = ([zboundlow.real + lines[len(lines) - 1]
                             [2], zboundlow.imag + lines[len(lines) - 1][3]])
        ...
        # the last line is empty
            ...
            if (lines[len(lines) - 2][2] != lines[len(lines) - 2][0]
                    and lines[len(lines) - 2][3] != lines[len(lines) - 2][1]):
                ...
                boundlow = ([zboundlow.real + lines[len(lines) - 2]
                             [2], zboundlow.imag + lines[len(lines) - 2][3]])
        ...
        for row in xrange(len(lines) - fx):
            ...
            # adjust it to that range and add compass heading to put the
            # linespol in compass domain
