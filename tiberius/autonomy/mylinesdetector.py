...
if (lines[len(lines) - 1][0] != 0):
    ...
    zboundlow = cmath.rect(
        gapvctpol[0], (gapvctpol[1] - curvfront / 180 * cmath.pi))
    boundhigh = ([zboundhigh.real + lines[len(lines) - 1][2],
                  zboundhigh.imag + lines[len(lines) - 1][3]])
    ...
    for row in xrange(len(lines) - fix):
        ...
        linespol.append([polstart[1] / cmath.pi * 180,
                         polstart[0], polend[1] / cmath.pi * 180, polend[0]])
else:
    print '2'
    ...
    zboundlow = cmath.rect(
        gapvctpol[0], (gapvctpol[1] - curvfront / 180 * cmath.pi))
    boundhigh = ([zboundhigh.real + lines[len(lines) - 2][2],
                  zboundhigh.imag + lines[len(lines) - 2][3]])
...
for x1, y1, x2, y2 in lines:
    cv2.line(img, (int(x1) + 700, int(y1) + 700),
             (int(x2) + 700, int(y2) + 700), (0, 0, 255), 1)
