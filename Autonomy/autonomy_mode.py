...
gpsh = 0
cmpsh = 0
srfdata = [] #used to store srf08 readings
#destination coordinates
dest_gps = []
dest_gps = getdata(mis_gps)
latend = dest_gps[0]
lonend = dest_gps[1]
gps_coord = [] #store the current gps coordinates
...
try:
    print 'activity 0'
    linespol = getdata(lidar)
    gpsh = gpsheading()      #heading towards the destination point
    cmpsh = getdata(cmps) #compass heading
    ...
    while (gpsdist()>1.21):
        ...
        if (activity==0):
            #state 0 checks if there is an obsticle on the line to the destination point
            if (state==0):
                print 'state 0'
                print 'lncnt: ',lncnt
                ...
                if (len(linespol)==1):
                    ...
                    if (gpsh-linestemp0>180): linestemp0+=360
                    elif (gpsh-linestemp0<-180): linestemp0-=360
                    if (gpsh-linestemp1>180): linestemp1+=360
                    elif (gpsh-linestemp1<-180): linestemp1-=360
                    #if the line is in the way or there is less than 85 cm between the path towards the destination and any of the line ends
                    if ((gpsh-linestemp0>0 and gpsh-linestemp1<0) or (poldist(linespol[0][2],linespol[0][3],gpsh,linespol[0][3])<85)\
                        ...
                        activity = 2
                        #checks which end of the line is closer and avoids the object from that side
                        if (linespol[0][1]<=linespol[0][3]):
                            ...
                            #between the robot and an obsticle when passing it by
                            ...
                            elif (driveangle<0): driveangle+=360
                        else:
                ...
                if (len(linespol)>1):
                    ...
                    #way of gpsh then another line will be introduced and the loop will be repeated
                    if (lncnt==len(linespol)):
                        angtemp = linespol[lncnt-1][2]+179.9
                        if (angtemp>360):
                            angtemp-=360
                        linespol.append([angtemp,linespol[lncnt-1][3],angtemp,linespol[lncnt-1][3]])
                        lncnt = 0
                    ...
                    if (gpsh-linestemp0>0 and gpsh-linestemp1<0):
                        ...
                        if (poldist(linespol[lncnt][2],linespol[lncnt][3],\
                            ...
                            if (linespol[lncnt][3]<=linespol[lncnt+1-fx][1]):
                                #if there is more than 85cm space between the line end and the path towards the destination
                                if (poldist(linespol[lncnt][2],linespol[lncnt][3],gpsh,\
                                            linespol[lncnt][3])>=85):
                                    activity = 1
                                #if there is no, then introduce a point to bypass the closest line
                                else:
                                    activity = 2
                                    drivetime = ((linespol[lncnt][3]+linespol[lncnt+1-fx][1])/2+addist)/52
            ...
            if (state==1):
                ...
                #check which passage is closer and also which passage is wide enough, go for the farer one if the closest one is not wide enough
                if ((math.fabs(gpsh-linestemp0)<math.fabs(gpsh-linestemp1) and poldist(linespol[lncntdn-1+fxdn][2],linespol[lncntdn-1+fxdn][3],\
                    ...
                    #check if the linespol(lncntdn-1) is in the way of driveangle
                    linestemp00 = linespol[lncntdn-1+fxdn][0]
                    linestemp11 = linespol[lncntdn-1+fxdn][2]
                    if (driveangle-linestemp00>180): linestemp00+=360
                    elif (driveangle-linestemp00<-180): linestemp00-=360
                    if (driveangle-linestemp11>180): linestemp11+=360
                    elif (driveangle-linestemp11<-180): linestemp11-=360
                    if (driveangle-linestemp00>0 and driveangle-linestemp11<0):
                        drivetime = ((linespol[lncntdn-1+fxdn][3]+linespol[lncntdn][1])/2+addist)/52
                    ...
                    if (driveangle-linestemp00>0 and driveangle-linestemp11<0):
                        drivetime = ((linespol[lncntup][3]+linespol[lncntup+1-fxup][1])/2+addist)/52
                        driveangle = linespol[lncntup+1-fxup][0]-math.acos((-(85*85)+math.pow(linespol[lncntup+1-fxup][1],2)\
                                            +math.pow(linespol[lncntup+1-fxup][1],2))/(2*math.pow(linespol[lncntup+1-fxup][1],2)))/math.pi*180
                        if (driveangle>=360): driveangle-=360
                        elif (driveangle<0): driveangle+=360
                #if reached the end or the beginning of the list then jump to the opposite end
                if (lncntdn==0):
                ...
                #introduces another line if there are two lines between which there is a gap >180 deg
                if (actlcnt==int(len(linespol)/2)+2): #checks if all the passages were evaluated
                    if (enclcnt==0):
                        actlcnt = 0
                        enclcnt+=1
                        ...
                        if (angtemp>360):
                            angtemp-=360
                        linespol.append([angtemp,linespol[len(linespol)-1][3],angtemp,linespol[len(linespol)-1][3]])
                        print 'introduced another line'
                        print linespol
        ...
        #if there were no obsticles detected in the way towards the destination point the robot turns to face the destination
        #and drives towards it adjusting its heding on the way by comparing compass data and gpsh
        if (activity==1):
            print 'activity 1'
            gpsh = gpsheading()
            cmpsh = getdata(cmps)
            if (gpsh-cmpsh>180): cmpsh+=360
            elif (gpsh-cmpsh<-180): cmpsh-=360
        ...
        if (activity==2):
            ...
            cmpsh = getdata(cmps)
            if (driveangle-cmpsh>180): cmpsh+=360
            elif (driveangle-cmpsh<-180): cmpsh-=360
