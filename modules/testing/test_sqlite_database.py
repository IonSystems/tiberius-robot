...
def clean_up():
    p = subprocess.Popen('find . -type f -name "test_db_*" -exec rm -f {} \;', shell=True)
...
class CreateTestDatabase(unittest.TestCase):
    ...
    def runTest(self):
		try:
			self.assertRaises(SqliteDatabase.OperationalError, sql.drop("test_table"))
...
class InsertDatabase(unittest.TestCase):
    def runTest(self):
        sql.insert("test_table", {'test_column': '100', 'test_column2': 'This is a test'})

class GenerateQueryString(unittest.TestCase):
    def runTest(self):
        specimen = sql._SqliteDatabase__generate_insert("insert", "test_table", {'test_column': '100', 'test_column2': 'This is a test'})
        control = "INSERT INTO test_table (test_column, test_column2) VALUES (100, 'This is a test')"
        self.assertEquals(specimen, control)

        specimen = sql._SqliteDatabase__generate_insert("insert or replace", "test_table", {'test_column': '100', 'test_column2': 200})
...
class GenerateSelectQuery(unittest.TestCase):
    def runTest(self):
        specimen = sql._SqliteDatabase__generate_query("select", 'test_table',"*", None)
        control = "SELECT * FROM test_table"
        self.assertEquals(specimen, control)

        specimen = sql._SqliteDatabase__generate_query("select", "test_table", 'test_column', None)
