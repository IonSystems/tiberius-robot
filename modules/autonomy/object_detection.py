    ...
    try:
        ...
        if (len(linespol)!=0):
                ...
                if (poldist(lines[0],lines[1],lines[2],lines[3])>=objsize-objtol and\
                    ...
                    if (firstcnt==1):
                        ...
                        if (lines[2]-lines[0]<0):
                            turnang = lines[2]-(lines[2]-lines[0]+360)/2
                        else:
                            turnang = lines[2]-(lines[2]-lines[0])/2
                        #creates an offset from the center of an object so that when the robot
                        #turns to that angle and gets rplidar data the first object will be this one
            ...
            for x in xrange(0,cnt):
                ...
                if (turnang-cmpsh>180): cmpsh+=360
                elif (turnang-cmpsh<-180): cmpsh-=360
                ...
                for lines in linespol:
                    ...
                    if (poldist(lines[0],lines[1],lines[2],lines[3])>=objsize-objtol and\
                        ...
                        if (firstcnt==1):
                            ...
                            #calculates an angle to face the center of an object
                            if (lines[2]-lines[0]<0):
                                driveangle = lines[2]-(lines[2]-lines[0]+360)/2
                            else:
                                driveangle = lines[2]-(lines[2]-lines[0])/2
                        if (scndcnt==2):
                ...
                #drives forward until the robot faces the object adjusting its path on the way
